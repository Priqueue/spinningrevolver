...
|---|---|---|---|
| `flip` | **无**（不发 `value`/`faceUp`/牌背） | **无** | 仅 `phase` 与 `turn`；**不下发任何行动记录**（谁已行动、选过哪些牌位都不发） |
| `swap` | **无** | **无** | 仅 `phase` 与 `turn`；**不下发易位牌号，也不下发非法尝试记录** |
...
**暗规则保护（本项目核心约束）**：翻转的判定逻辑（同状态翻自己 / 异状态与对手交换）、易位的合法性条件（同明暗状态才可易位）、结算算法，都属于**玩家不可直接得知**的规则。因此：

1. 易位的牌号**不公开**——公开它会暴露「同状态才可易位」这一条件，玩家可据此反推双方牌的明暗分布。
2. 翻转/易位阶段**不下发任何行动记录**（`acted` 类字段）——它会让玩家推导出对手的行动进度与所选牌位。
3. 公开日志只记录「完成了翻转 / 完成了易位」这类中性事实；牌号与非法尝试细节只写**本人可见**的私有日志。
4. 客户端**不向玩家解释上述规则**：大厅「玩法速览」不提及牌面点数组成、翻转判定、易位条件与结算算法；对局中的操作提示只说明「请选择一个牌位」，由玩家通过服务端反馈自行推断。

#### 5.1.1 翻转阶段：双方都观测不到牌面变化（对称性）

**约束**：翻转阶段中，**行动者本人也无法观测到自己的牌发生了何种变化**——不能知道系统执行的是「翻自己的牌」还是「与对手交换」。

实现保证：

- 翻转阶段不下发牌对象、`value`、`faceUp`，`faceUpTotal` 为 `null`。
- 翻转动作后，行动者视图与动作前**逐字段同构**（round / phase / status / opponent / bet 完全相同），
  唯一变化是 `turn` 交予对方、`youActed` 变为 `true`。
- 双方在翻转阶段拿到的**可观测信息完全对称**，不因身份不同而不同（`turn` 与 `youActed` 属各自视角）。

> 由 `tests/rules.view.test.ts` 的「行动者自己也察觉不到自己的牌发生了变化」「翻转阶段双方可观测信息完全对称」两条测试守护。

#### 5.1.2 结算结果的两个字段被裁剪

`lastBrightOwner`（最后被计入的明牌归属）与 `tieBreakStarter`（破平起始方）**不下发客户端**。
它们会指出某张牌的状态，而该牌位置在翻转阶段不可观测，下发等于绕过 5.1.1 的约束。
服务端内部仍完整保留这两个字段用于判定与日志；下行类型为 `SettlementView`（`Omit<SettlementResult, 'lastBrightOwner' | 'tieBreakStarter'>`）。
