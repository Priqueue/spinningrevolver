...
| 阶段 | 自己可见 | 对手可见 | 系统额外告知 |
|---|---|---|---|
| 翻转 | 无任何牌信息 | 无 | 仅阶段与行动者；**不公开任何行动记录** |
| 易位 | 无任何牌信息 | 无 | 仅阶段与行动者；**不公开易位牌号，也不公开非法尝试** |
...
翻转的判定逻辑（同状态翻自己 / 异状态与对手交换）、易位的合法性条件（同明暗状态才可易位）、
结算算法，都属于**玩家不可直接得知**的规则。为此：

- **易位牌号不公开**：公开它会暴露"同状态才可易位"，玩家可据此反推双方牌的明暗分布。
- **翻转/易位阶段不下发任何行动记录**：否则玩家能推导出对手的行动进度与所选牌位。
- **翻转阶段双方都观测不到牌面变化**：连行动者本人也不知道系统是"翻了自己的牌"还是"与对手交换了" ——
  翻转动作前后，行动者的视图逐字段同构，双方可观测信息完全对称。
- **结算结果裁剪两个字段**：`lastBrightOwner` 与 `tieBreakStarter` 不下发，它们可反推某张牌的状态。
- **公开日志只写中性事实**（"完成了翻转""完成了易位"）；牌号与非法尝试细节只进**本人可见**的私有日志。
- **界面不解释暗规则**：大厅「玩法速览」不提及牌面点数组成、翻转判定、易位条件、结算算法；
  对局中只提示"请选择一个牌位"，玩家需通过服务端反馈（阶段推进、非法提示）自行推断规则。

UI 上唯一的进度反馈是「已记录你的选择，等待对手」——它只依据**你自己**是否已行动（`youActed`）来措辞，
不包含任何对手状态，因此不会泄露对手的牌位选择。

这些约束由测试守护：`tests/rules.view.test.ts`（视图投影层，含对称性断言）与
`npm run test:integration` 中的协议级扫描（60 项，逐帧检查不含 `slots`/`attempts`/`faceUp`/`lastBrightOwner` 等字段）。
